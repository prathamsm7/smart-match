...
export const jobsService = {
    async fetchJobs(options: FetchJobsOptions = {}) {
        ...
        if (options.myJobs) params.append('myJobs', 'true');
    ...
    async fetchJobMatches(options: FetchJobMatchesOptions = {}) {
        const params = new URLSearchParams();
        if (options.mode) params.append('mode', options.mode);
        if (options.query) params.append('query', options.query);
        if (options.forceRefresh) params.append('forceRefresh', 'true');

        const url = `/api/jobs/matches${params.toString() ? `?${params.toString()}` : ''}`;
        const response = await fetch(url, { cache: 'no-store' });
        const data = await response.json();
        if (!response.ok) throw new Error(data.error || 'Failed to fetch job matches');
        return data;
    },
};
