...
const FEATURE_CONFIG: Record<string, { label: string; icon: React.ReactNode }> = {
    ...
    job_posting: { label: 'Job Postings', icon: <Briefcase className="w-4 h-4" /> },
    interview_report_view: { label: 'Interview Reports', icon: <FileText className="w-4 h-4" /> },
};
    ...
    return (
        <div className="max-w-6xl mx-auto space-y-8">
            ...
            <div className="flex items-center gap-3">
                <div className={`w-10 h-10 ${isRecruiter ? 'bg-gradient-to-br from-indigo-500 to-blue-600 shadow-blue-500/30' : 'bg-gradient-to-br from-violet-500 to-purple-600 shadow-purple-500/30'} rounded-xl flex items-center justify-center shadow-lg`}>
                    <CreditCard className="w-5 h-5 text-white" />
                </div>
                <div>
                    <h2 className="text-xl font-bold text-white">Billing & Subscription</h2>
                    <p className="text-sm text-gray-400">
                        {isPaid ? `You're on the ${plan.displayName} plan` : 'Upgrade to unlock more features'}
                    </p>
                </div>
            </div>
            ...
            <div>
                ...
                {isRecruiter ? (
                    <RecruiterPlans plan={plan as any} upgrading={upgrading} onUpgrade={upgrade} />
                ) : (
                    <CandidatePlans plan={plan as any} isPaid={isPaid} upgrading={upgrading} onUpgrade={upgrade} />
                )}
            </div>
